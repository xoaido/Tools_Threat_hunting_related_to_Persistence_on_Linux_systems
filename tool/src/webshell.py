...
def webshellScanner(path, valid_regex):
    ...
    print(colorama.Fore.LIGHTRED_EX + "\n[[ Top cumulative ranked files ]]" + colorama.Fore.RESET)
    count = 10
...
class SignatureInLog:
   ...
   def calculate(self, data, filename):
       ...
       valid_regex = re.compile('(system|exec|shell_exec|passthru|popen|proc_open|\\bin\\bash|\\bin\\sh)', re.I)
       count = 0
       for file in files:
            if 'access' in file:
               # Open file log
   ...
   def printer(self, count):
       """Print the top signature count match files for a given search"""
       print("\n[[ Top %i signature match counts in log access (High possibility of bad!) ]]" % (count))
       if (count > len(self.results)): count = len(self.results)
       for x in range(count):
           print(' {0:>7}        {1}'.format(self.results[x]["value"], self.results[x]["filename"]))
       return
